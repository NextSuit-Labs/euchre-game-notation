...
EGN v1.2.0 is fully backward compatible with v1.0.0 and v1.1.0:
...
- Use the `egn-upgrade` CLI tool to migrate files to v1.2.0, then manually add Player Object data as needed
...
// After (v1.2.0 with external IDs)
{
  "metadata": {
    "players": [
      {
...
Player Objects in EGN v1.2.0 enable seamless cross-platform player tracking while maintaining backward compatibility with simple player names. This feature is essential for building robust tournament systems, competitive analytics, and unified game archives across the Euchre community.
